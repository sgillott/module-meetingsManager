...
if (empty($meetingsManagerOccurrenceID) || empty($meetingsManagerDefinitionID)) {
    $URL .= '&return=error1';
    header("Location: {$URL}");
    exit;
}
...
try {
    $container->get(MeetingExceptionGateway::class)->deleteWhere(['meetingsManagerOccurrenceID' => $meetingsManagerOccurrenceID]);

    // Restores the Calendar event to the normal planned schedule/status immediately.
