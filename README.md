...
right dates - automatically. It reuses Gibbon's own Calendar, timetable, staff and department data
rather than keeping a separate copy of any of it, so generated meetings appear on every
...
- [Archiving a meeting series](#archiving-a-meeting-series)
...
- Lets you describe a meeting once - name, organiser, schedule, audience - as a **Meeting
  Definition**.
...
- Works out who should attend by re-checking Gibbon's actual staff, department and year group data
  every time, not by keeping a fixed list of names.
- Creates and keeps in sync the matching native Gibbon Calendar events, so meetings show up on
  every participant's Calendar and Timetable the normal way.
...
- The "Manage Meetings" permission (see [Permissions](#permissions)) to create and manage meetings.
...
4. Go to **Other > Meetings Manager Settings** and confirm the Calendar Event Type (see
...
- **Meeting Definition** - what you configure: the meeting's name, organiser, schedule, and who
  should attend. This is what you see and edit in Meetings Manager.
...
1. Go to **Other > Manage Meetings** and click **Add Meeting**.
2. Give it a name, confirm the organiser (defaults to you), and choose **Once** as the schedule
   type, with a date and time.
3. Save. You'll land on the Edit page.
...
| **Archived** | Archived (see [Archiving a meeting series](#archiving-a-meeting-series)) |
...
**Other > Meetings Manager Settings** lets you choose which native Gibbon Calendar Event Type
...
Go to **Other > Manage Meetings > Add Meeting**. You'll configure:

**Basic details** - Name (required), Description and Location (optional), and Organiser (defaults
to you; pick any other active staff member instead if needed).
...
On the Edit page, under **Audience**, add one or more rules describing who should attend. You can
combine as many as you like - the final list is everyone matched by any rule, minus anyone
excluded:
...
Picking several Year Groups, Departments, or Staff at once adds one rule per selection in a single
step - you don't need to repeat the form for each one.
...
context for you to act on if needed, using the controls below.

Two quick actions are also available directly from Preview, without needing to open Edit:

- **Exclude a date** - tick off the checkbox next to any candidate date (e.g. one that clashes with
  a Parent-Teacher Conference week) to stop it from ever generating a meeting. The row's background
  updates immediately, and the choice is remembered for future previews and generations, until you
  tick it back on. A School Closure day can't be overridden here, since the school genuinely isn't
  open.
- **Add or remove a participant** - use **Add Participant** below the participant list to include
  someone specific, or select one or more people in the participant list and click **Remove** to
  take them out. Both act the same as adding a Specific Staff or Exclude Individual audience rule
  from Edit - they're just quicker to reach from here.
...
|---|---|---|
| **Manage Meetings** | Create, edit, preview, generate, and archive meetings | Administrator |
...
Like any Gibbon module, Manage Meetings can be granted to other roles from **Admin > User Admin >
Roles & Permissions** if your school wants specific staff (e.g. a Leadership Team or Office Manager
role) to be able to manage meetings without full Administrator access.
...
The Meeting Definition and its full history are preserved under the Archived filter - nothing is
permanently deleted. There's no "un-archive to Active" action in this version; contact an
Administrator if you need the underlying configuration restored to active use.
